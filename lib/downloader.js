...
/**
 * Download gambar/thumbnail dari URL sosmed menggunakan yt-dlp.
 *
 * @param {string} url - URL post sosmed
...
async function downloadImage(url) {
    ...
    // Download thumbnail menggunakan yt-dlp
    const cmd = [
        'yt-dlp',
        `"${url}"`,
        '--write-thumbnail',
    ...
    await new Promise((resolve, reject) => {
        exec(cmd, { timeout: 60000, maxBuffer: 50 * 1024 * 1024 }, (error, stdout, stderr) => {
            if (error) {
                console.log(`[DL-IMG] Error: ${error.message}`);
                if (stderr) console.log(`[DL-IMG] Stderr: ${stderr.substring(0, 300)}`);
                ...
                return reject(new Error('Gagal download gambar. Pastikan URL valid.'));
            }
            resolve();
        });
    });

    // Cari file gambar hasil download
    const imgFile = findDownloadedFile(tmpId);

    if (!imgFile) {
        cleanupTempFiles(tmpId);
        throw new Error('Gambar tidak ditemukan. URL mungkin tidak memiliki thumbnail.');
    }
...
module.exports = { downloadMedia, downloadImage, detectPlatform, formatDuration };
