...
async function addExifToWebp(webpBuffer) {
    try {
        ...
        const exifData = buildStickerExif(STICKER_AUTHOR);
        img.exif = exifData;

        return await img.save(null);
    } catch (err) {
        // Jika EXIF injection gagal, kembalikan WebP tanpa EXIF
        console.log('EXIF injection gagal, kirim tanpa metadata:', err.message);
        return webpBuffer;
    }
}
 ...
 */
function buildStickerExif(author) {
    const json = JSON.stringify({
        'sticker-pack-id': 'com.va.bot',
        'sticker-pack-name': '',
...
/**
 * Convert stiker WebP ke gambar PNG menggunakan ffmpeg.
 *
 * @param {Buffer} webpBuffer - Buffer stiker WebP
 * @returns {Promise<Buffer>} - Buffer gambar PNG
 */
async function stickerToImage(webpBuffer) {
    const tmpId = crypto.randomBytes(8).toString('hex');
    const tmpDir = os.tmpdir();
    const inputPath = path.join(tmpDir, `va_s2i_${tmpId}.webp`);
    const outputPath = path.join(tmpDir, `va_s2i_${tmpId}.png`);
    ...
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            execFile('ffmpeg', args, { timeout: 15000 }, (error) => {
                if (error) return reject(new Error('Gagal convert stiker ke gambar.'));
                if (!fs.existsSync(outputPath)) return reject(new Error('File gambar tidak dibuat.'));
                resolve();
            });
        });
...
/**
 * Ganti author name pada stiker WebP.
 * Re-inject EXIF metadata dengan author baru.
 *
 * @param {Buffer} webpBuffer - Buffer stiker WebP asli
 * @param {string} newAuthor - Nama author baru
 * @returns {Promise<Buffer>} - Buffer stiker dengan author baru
 */
async function changeStickerAuthor(webpBuffer, newAuthor) {
    const img = new webpmux.Image();
    await img.load(webpBuffer);

    const exifData = buildStickerExif(newAuthor);
    img.exif = exifData;

    return await img.save(null);
}

module.exports = { createSticker, createGifSticker, stickerToImage, changeStickerAuthor };
